// Copied from github.com/paoloricciuti/sveltekit-search-params
        ...
        setRef.value = (value) => {
            ...
            const toBatch = (query: URLSearchParams) => {
                for (const field of Object.keys(value)) {
                    ...
                    let newVal = fnToCall((value as any)[field]);
                    if (newVal == undefined) {
                        query.delete(field as string);
                    } else {
                        query.set(field as string, newVal);
                    }
                }
            };
