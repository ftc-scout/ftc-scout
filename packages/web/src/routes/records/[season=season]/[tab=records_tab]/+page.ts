import { DESCRIPTORS, getMatchStatSet, getTepStatSet, type Season } from "@ftc-scout/common";
...
export const load: PageLoad = ({ fetch, params, url }) => {
    ...
    if (params.tab == "teams") {
        ...
        let sortBy = STAT_EC_DC(
            stats,
            descriptor.pensSubtract ? "totalPointsThis" : "totalPointsNpThis"
        ).decode(url.searchParams.get("sort"));
        let sortDir = SORT_DIR_EC_DC.decode(url.searchParams.get("sort-dir")) as SortDirGQL;
        ...
        return {
            matchData: getData(getClient(fetch), MatchRecordsDocument, {
                season,
                skip,
                take,
                sortBy,
