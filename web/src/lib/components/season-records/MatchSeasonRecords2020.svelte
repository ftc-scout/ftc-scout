<script lang="ts" context="module">
    ...
    type Data = FullScores2020Shared;
    ...
    export const DEFAULT_SORT_MATCH_2020: ChosenSort<Data> = { stat: THIS_TOTAL_POINTS_STAT, type: SortType.HIGH_LOW };

    let currentFilters: Writable<Filter<Data>> = writable(emptyFilter());

    let currentSort: Writable<ChosenSort<Data>> = writable(DEFAULT_SORT_MATCH_2020);

    export function getStatSet2020Matches(): StatSet<unknown, unknown> {
        return [...STAT_SET_MATCHES_2020_SHARED];
    }
    ...
    export let match2020SearchParams: string = "";

    export function resetMatch2020SearchParams() {
        shownStats.set([
            THIS_TOTAL_POINTS_STAT,
            THIS_AUTO_POINTS_STAT,
            THIS_DC_POINTS_STAT,
        ...
        currentSort.set(DEFAULT_SORT_MATCH_2020);
...
<script lang="ts">
    ...
    let statSet: StatSet<unknown, unknown> = getStatSet2020Matches();
    $: statSet = getStatSet2020Matches();
    ...
    $: isDefaultSort =
        $currentSort.type == DEFAULT_SORT_MATCH_2020.type &&
        $currentSort.stat.identifierName == DEFAULT_SORT_MATCH_2020.stat.identifierName;
    ...
    $: if ($page.params.tab == "matches") match2020SearchParams = $page.url.searchParams.toString();
...
<StatsTable
    ...
    defaultSort={DEFAULT_SORT_MATCH_2020}
    bind:currentSort={$currentSort}
    bind:currentFilters={$currentFilters}
    fileName={"Match Season Records 2020"}
    pagination
    bind:page={currPage}
    {totalCount}
    {pageSize}
