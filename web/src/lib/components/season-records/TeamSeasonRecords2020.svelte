<script lang="ts" context="module">
    ...
    export const DEFAULT_SORT_TEAM_2020: ChosenSort<Data> = { stat: OPR_STAT, type: SortType.HIGH_LOW };

    let currentFilters: Writable<Filter<Data>> = writable(emptyFilter());

    let currentSort: Writable<ChosenSort<Data>> = writable(DEFAULT_SORT_TEAM_2020);

    export function getStatSet2020Teams(_eventTypes: EventTypes): StatSet<unknown, unknown> {
        return [...STAT_SET_TEAMS_2020_SHARED, ...STAT_SET_EVENT_2020];
    }
    ...
    export let team2020SearchParams: string = "";

    export function resetTeam2020SearchParams() {
        shownStats.set([
            TEAM_STAT,
            OPR_STAT,
            NP_OPR_STAT,
        ...
        currentSort.set(DEFAULT_SORT_TEAM_2020);
...
<script lang="ts">
    ...
    import { EVENT_STAT_2020, STAT_SET_EVENT_2020 } from "$lib/util/stats/StatsEvent";
    ...
    import type { FullTep2020Shared } from "../../util/stats/2020/StatsSharedTeams2020";
    ...
    let statSet: StatSet<unknown, unknown> = getStatSet2020Teams(eventTypes);
    $: statSet = getStatSet2020Teams(eventTypes);
    ...
    $: isDefaultSort =
        $currentSort.type == DEFAULT_SORT_TEAM_2020.type &&
        $currentSort.stat.identifierName == DEFAULT_SORT_TEAM_2020.stat.identifierName;
    ...
    $: if ($page.params.tab == "teams") team2020SearchParams = $page.url.searchParams.toString();
...
<StatsTable
    ...
    defaultSort={DEFAULT_SORT_TEAM_2020}
    bind:currentSort={$currentSort}
    bind:currentFilters={$currentFilters}
    fileName={"Team Season Records 2020"}
    pagination
    bind:page={currPage}
    {totalCount}
    {pageSize}
