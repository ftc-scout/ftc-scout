<script lang="ts">
    import { TournamentLevel } from "../../graphql/generated/graphql-operations";
...
<thead>
    ...
    <div class="centerer">
        <span>
            ...
            {:else if level == TournamentLevel.Semis}
                Semifinals
            {:else if level == TournamentLevel.Finals}
                Finals
            {/if}
        </span>
    </div>
...
<style>
    ...
    .blue-block {
        position: absolute;
        left: calc(10.75em + (100% - 10.75em) / 2);
        width: calc((100% - 10.75em) / 2);
        height: 100%;
